...
exports.create = function(req, res, callback){
	...
	console.log('--------------req----------------');
	//console.log(req);
         new con.Request("insert into [users] (name, username, password, email, birthday, lat, lon, created_at, updated_at, verification_code, photo_url, azure_id, social_access_token, phone) values( @name, @username, @password, @email, @birthday, @lat, @lon, @created_at, @updated_at, @verification_code, @photo_url, @azure_id, @social_access_token, @phone); select * from [users] where id = scope_identity();") 
            ...
            .addParam("phone", TYPES.VarChar, req.body.phone)
...
exports.update = function(req, res, callback){
    ...
    //Password is opcional
    let password = req.body.password || false, query = "";
    if( password !== false ){
        password = (MD5(password));
        query = String.raw`
        update [users] set name=@name, username=@username, phone=@phone, password=@password, email=@email, 
        birthday=@birthday, lat=@lat, lon=@lon, photo_url=@photo_url, updated_at=@updated_at, 
        social_access_token=@social_access_token  
        where id = @id; select * from [users] where id = @id;
        `
    }else{
        query = String.raw`
        update [users] set name=@name, username=@username, phone=@phone, email=@email, 
        birthday=@birthday, lat=@lat, lon=@lon, photo_url=@photo_url, updated_at=@updated_at, 
        social_access_token=@social_access_token  
        where id = @id; select * from [users] where id = @id;
        `
    ...
    con.connect().then(function () { 
        ...
        .addParam("name", TYPES.VarChar, req.body.name) 
        .addParam("username", TYPES.VarChar, req.body.username) 
        .addParam("password", TYPES.VarChar, password) 
        .addParam("phone", TYPES.VarChar, req.body.phone) 
        .addParam("email", TYPES.VarChar, req.body.email) 
        .addParam("birthday", TYPES.VarChar, req.body.birthday)
        .addParam("lat", TYPES.VarChar, req.body.lat)
